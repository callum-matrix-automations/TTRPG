...
NPCs exist on a tier spectrum that determines how much data they have, what the UI shows, and whether the AI generates full stats for them. **Tiers only promote upward** — once stats are generated, they are never downgraded or regenerated (unless the Orchestrator explicitly requests an update).

### Tier Definitions

| Tier | Definition | Stats Generated? | UI Shows | Promotion Path |
|------|-----------|-------------------|----------|----------------|
| **Background** | Set dressing. May not have a name. | No stats at all | Name + one-line description | → Passive (on player engagement) |
| **Passive** | Has a face and role but doesn't mechanically participate | Basic only: level, HP, XP, faction | Portrait, disposition, faction, traits, appearance | → Active (on mechanical engagement — roll initiated, combat, etc.) |
| **Active** | Participates in mechanics — rolls dice, can be rolled against, has goals that drive action | **Full stat block generated by AI**: class, abilities, skills, stats, status effects | What the player would observe: impression, observed capabilities, visible status. Full stats hidden from player. | → Companion (on joining the party) |
| **Companion** | Travels with the player, player has some control | Full stats (same as Active) + gear, loyalty | Full character sheet in Party panel — HP, class, stats, skills, gear, abilities, status effects | Terminal tier |

### Stat Generation Triage

The AI generates stats **only when an NPC is promoted to Active tier**. This saves context and computation:

- **Background → Passive**: AI assigns basic stats (level, HP, XP) based on the NPC's role and the scene context. No full stat block.
- **Passive → Active**: AI generates the full stat block (class, all 6 ability scores, skills, proficiencies, abilities). This is a one-time generation triggered by the Orchestrator when the NPC needs to mechanically participate (initiates a roll, enters combat, player attempts a check against them, etc.).
- **Active → Companion**: No new stat generation needed — the stats already exist. The Orchestrator adds gear and loyalty tracking.

### What "Active" Means

Active does **not** mean hostile. An Active NPC is any NPC that participates in the game's mechanics:
- A suspicious guard rolling Insight against the player's Deception
- A friendly ally fighting alongside you in combat (but not in your party)
- A rival making contested checks against you in a card game
- A merchant who might notice you stealing (Perception check)
- A healer casting a spell to help you (requires stats for the Resolution Engine)

### Information Asymmetry

The player never sees an Active NPC's raw stat block. Instead, the UI shows **observations**:
- `observedCapabilities`: narrative descriptions derived from stats ("Quick hands", "Sharp eye for detail")
- `threatImpression`: a gut-feel assessment based on the player character's INT/WIS ("Could be dangerous", "Not a threat")
- `visibleStatus`: only externally observable conditions ("Wounded", "Alert", "Drunk") — not internal states

This matches real TTRPG play where the GM describes NPCs narratively, not numerically.
